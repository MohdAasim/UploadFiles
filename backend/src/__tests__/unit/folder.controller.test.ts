...
describe('Folder Controller', () => {
  ...
  describe('createFolder', () => {
    it('should create a folder successfully', async () => {
      ...
      // Assert
      expect(addFolder).toHaveBeenCalledWith('New Folder', 'parent123', 'user123');
    ...
    it('should create a root folder when parent is not provided', async () => {
      ...
      // Assert
      expect(addFolder).toHaveBeenCalledWith('Root Folder', undefined, 'user123');
      expect(mockRes.status).toHaveBeenCalledWith(201);
      expect(mockRes.json).toHaveBeenCalledWith(mockFolderResponse);
    });
  });
  ...
  describe('listFolderTree', () => {
    ...
    it('should list folders and files for specific parent folder', async () => {
      ...
      const mockFolders = [
        { _id: 'subfolder1', name: 'Sub Folder 1', owner: 'user123', parent: parentFolderId }
      ];
      
      const mockFiles = [
        { _id: 'file1', originalName: 'file1.pdf', uploadedBy: 'user123', parentFolder: parentFolderId }
      ];
  ...
  describe('getAllFolders', () => {
    ...
    it('should list all user folders', async () => {
      ...
      const mockFolders = [
        ...
        { _id: 'folder2', name: 'Folder 2', owner: 'user123', parent: 'folder1' }
      ];
  ...
  describe('deleteFolder', () => {
    ...
    it('should delete folder recursively with all contents', async () => {
      ...
      (FileMeta.find as jest.Mock).mockImplementation((query) => {
        ...
        if (query.parentFolder === 'subfolder123' && query.uploadedBy === userId) {
          return Promise.resolve([]);
        }
        return Promise.resolve([]);
      });
