...
export class SocketServer {
  ...
  private connectedUsers: Map<string, { socketId: string; userData: any }> = new Map();
  public fileEditingSessions: Map<string, { userId: string; userName: string; timestamp: Date }> = new Map();
  ...
  private handleFileEvents(socket: AuthenticatedSocket) {
    ...
    // File uploaded
    socket.on('file-uploaded', (data: { fileData: any; parentFolder?: string }) => {
    ...
    socket.on('file-version-updated', (data: { fileId: string; versionData: any }) => {
  ...
  private handleSharingEvents(socket: AuthenticatedSocket) {
    // File/folder shared
    socket.on('resource-shared', (data: { 
      resourceId: string; 
  ...
  private handleCollaborationEvents(socket: AuthenticatedSocket) {
    ...
    socket.on('cursor-position', (data: { resourceId: string; position: any }) => {
  ...
  private handleNotificationEvents(socket: AuthenticatedSocket) {
    ...
    socket.on('get-online-users', () => {
      const onlineUsers = Array.from(this.connectedUsers.values()).map(user => user.userData);
  ...
  public notifyFileShared(resourceId: string, resourceType: 'file' | 'folder', targetUserId: string, sharedBy: any, permission: string) {
  ...
  public notifyResourceDeleted(resourceId: string, resourceType: 'file' | 'folder', deletedBy: any) {
