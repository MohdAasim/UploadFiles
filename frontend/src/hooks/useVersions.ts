  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['version-history', variables.fileId],
      });
    ...
    onError: (error) => {
      ...
      toast.error(
        `Failed to upload new version: ${error?.message || 'Unknown error'}`
      );
    },
  });
};
...
export const useRestoreVersion = () => {
  ...
  return useMutation({
    mutationFn: (data: { fileId: string; versionNumber: number }) =>
      versionsAPI.restoreVersion(data.fileId, data.versionNumber),
    onSuccess: (_, variables) => {
      toast.success(
        `Version ${variables.versionNumber} restored successfully!`
      );
      // Invalidate relevant queries
      queryClient.invalidateQueries({
        queryKey: ['version-history', variables.fileId],
    ...
    onError: (error) => {
      ...
      toast.error(
        `Failed to restore version: ${error?.message || 'Unknown error'}`
      );
    },
  });
};
